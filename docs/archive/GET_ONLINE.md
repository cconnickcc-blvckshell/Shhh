...
| **Twilio works in Console but not from app** | **Trial account:** You can only send to numbers verified in [Twilio Console → Verified Caller IDs](https://www.twilio.com/console/phone-numbers/verified). Add each user's number there, or upgrade to paid. Also: `TWILIO_PHONE_NUMBER` must be your trial/sandbox number. |
...
| **429 Too Many Requests** | Global: 2000 req/15min default. Admin dashboard + polling can use 50+ req/min. Set `RATE_LIMIT_MAX_REQUESTS=5000` in Render if still hitting limits. Auth: 30 attempts/15min; set `AUTH_RATE_LIMIT_MAX=50` if needed. |
| **Two accounts can't see each other in Discover** | 1) **Web vs mobile:** Web now uses browser geolocation; grant location permission. Previously web defaulted to NYC. 2) **Radius:** Default is now "All" (100km). Use the radius chip to cycle 5→25→50→All. 3) **Location:** Both must have opened the app so location was updated. 4) **Mutual filters:** Seeking preferences must match (NULL = match all). |
