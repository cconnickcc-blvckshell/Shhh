...
The test suite contains **93 tests** across 12 suites: `auth`, `analytics`, `referrals`, `discovery`, `events` (including Tonight feed), `couples`, `safety`, `admin`, `media`, `visibility`, `verification`, `messaging`. Uses **Jest** with **Supertest** for HTTP assertions.
...
**useSocket** also returns `connected`, `reconnecting` for state transparency. Chat shows "Reconnecting…" banner when `reconnecting`. **Chat:** Optimistic message send; failed messages show "Tap to try again" with retry. **Discover:** Swipe right/left for like/pass; left-edge swipe to Messages. **User profile:** X icon → "Not interested" (pass with optional reason) or "Block". **Accessibility:** accessibilityLabel/accessibilityRole on key buttons (Like, Message, Whisper, Send, Save, RSVP, empty CTAs, Panic, Log out, MenuItem). SafeState has accessibilityRole="summary". social proof bar ("X people nearby"); variable reward toast on refresh ("X new people nearby"). **Profile (Me):** User ID row with copy. **Edit Profile:** Card sections, bio char count (500), isDirty + unsaved-changes warning on back. **User profile:** Privacy cue badges (after_match, after_reveal). **Whispers:** "Anonymous" badge when sender not revealed. **Create venue:** "Use my location" button. **Create event / Event edit:** Native DateTimePicker (web: TextInput); "Show advanced options" for visibility, tier, radius, location revealed. **Edit Profile:** "Show advanced options" for Discovery & Privacy, Hosting; Primary vibe in About. **Photo upload:** Progress bar (edit profile, album add). **Admin Dashboard:** Tier Funnel card (Signups → Verified → Premium); Trust Score Distribution histogram (0-20, 21-40, 41-60, 61-80, 81-100, N/A). **Whispers:** Swipe-left on pending inbox whisper = Ignore (native). **mapApiError:** 403/401, whisper expired, profile not found. **Wave 14 (P0/P1):** Unread sync: refetch on WebSocket notification, AppState active; POST /conversations/:id/read when viewing chat + new message; Onboarding: "Browse first" primary CTA; Push throttle: max 1 per user per 30s.
...
| `/v1/referrals` | `referralRoutes` | `modules/referrals/referral.routes.ts` |
