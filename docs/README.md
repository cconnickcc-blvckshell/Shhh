...
## 10 Master Documents (Consolidated)
...
| 5 | **AUDIT_AND_STATUS.md** | Index to audit reports, implementation status |
...
|----------|---------|---------------------|
| **ARCHITECTURE.md** | System overview, file tree, API ledger (§4), schema (§5), dependencies, workflows. | When adding routes, modules, or tables: update §2, §4, §5, §6. |
| **DEV_HANDOVER.md** | Deep reference: every backend module (§4), schema, mobile routes and API, real-time, workers, safety, billing, env (§18). | When changing a module or endpoints: update the corresponding §4.x and schema/API tables. |
...
| Document | Purpose | When to read/update |
|----------|---------|---------------------|
| **AUDIT_AND_STATUS.md** | **Index** — Entry point for all audit reports and implementation status. | When assessing status or finding the right audit doc. |
...
| **ROADMAP.md** | **Master roadmap** — Me section, admin dashboard, game-changers. Status summary. | When planning work or checking what's done vs pending. |
| **GAME_CHANGER_ROADMAP.md** | Detailed game-changer ideas; themed phases. | When picking or implementing a game-changer; update after build. |
...
| Document | Purpose | When to read/update |
|----------|---------|---------------------|
| **OPS_GUIDE.md** | **Single ops reference:** Get online (cloud setup), runbook (incidents/debugging), testing, deployment, alerting. | When deploying, on-call, adding tests, or troubleshooting. |
...
| **ME_SECTION_SPRINT_REPORT.md** | *(Archived)* Me section sprint report. See archive. | — |
